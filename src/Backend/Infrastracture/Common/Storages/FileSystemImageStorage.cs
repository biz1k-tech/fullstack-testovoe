...
public class FileSystemImageStorage : IFileSystemImageStorage
{
    ...
    public async Task<(string FullImagePath, string ThumbnailPath)> SaveImageWithResizeAsync(Guid id,
        ...
        CancellationToken cancellationToken)
    {
        using var fullImageTask =
            SaveStreamToFileAsync(id, ImageSuffix.Original, originalStream, extension, cancellationToken);
        using var thumbnailTask =
            SaveStreamToFileAsync(id, ImageSuffix.Resize, thumbnailStream, extension, cancellationToken);
